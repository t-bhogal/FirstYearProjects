...
public class HeapSort {
    ...
    private static void heapSort(int[] _arr) {
        ...
        for (int i = n / 2 - 1; i >= 0; i--) //goes through each tree
             heapify(_arr, i, n);
    ...
    private static void heapify(List<CosSimilarityPair> _tree, int _rootindex, int _n) {
        while (2 * _rootindex + 2 <= _n) {
            int leftIndex = 2 * _rootindex + 1;
            int rightIndex = 2 * _rootindex + 2;
            int smallest = leftIndex;

            if (rightIndex < _n && _tree.get(leftIndex).getCosineSimilarity() > _tree.get(rightIndex).getCosineSimilarity())
                smallest = rightIndex;
